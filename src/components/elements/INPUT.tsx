        ...
        <div className='flex flex-col gap-y-2 text-Regular-Normal-text-1'>
            { label ? <label>{ label }</label> : null}
            {
                !textarea ? <div className='flex items-center relative text-Neutral-800 '>
                                <input
                                    className={`${style ? style : ""} w-full text-Regular-Normal-text-2 p-3 border border-Neutral-400 rounded-lg focus:text-Neutral-900 focus:border-Neutral-900 focus:outline-none`} 
                                    type={ newType }
                                    value={ value }
                                    name={ name }
            ...
            { error ? <span className='text-Regular-Caption-1 lg:text-Regular-Normal-text-2 ml-4 text-Error-400'>{error}</span> : null}
        </div>
    );
};
